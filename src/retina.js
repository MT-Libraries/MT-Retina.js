...
(function () {
    ...
    var processImg = function () {
        ...
        for (var i = 0; i < allImgs.length; i++) {
            ...
            var url = allImgs[i].getAttribute(DATA_ENUM.IMG_URL);

            if (url) {

                var dataImgType = allImgs[i].getAttribute(DATA_ENUM.IMG_TYPE);
                var dataImgLang = allImgs[i].getAttribute(DATA_ENUM.IMG_LANG) || 'zh';
                                       
                if (_imgTypeConfigs[dataImgType]) {

                    var config = _imgTypeConfigs[dataImgType];
                    var lang = _imgLangConfigs[dataImgLang];
                    ...
                    if (filterResult.filter) {
                        ...
                        var result = filter(url, config[platform].base, config[platform][_devicePixelRatio], lang.lang);

                        var defaultUrl;

                        if (config[platform]['default']) {
                            defaultUrl = filter(config[platform]['default'], config[platform].base, config[platform][_devicePixelRatio], lang.lang);
                        }
    ...
    Retina.setConfigs = function (currentPlatform, platforms, configs, langs) {

        _currentPlatform = currentPlatform;
        _imgTypeConfigs = configs;
        _imgLangConfigs = langs;
